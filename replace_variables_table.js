...
const fileToModify = path.join(process.cwd(), 'src', 'pages', 'HonorariosVariablesView.jsx');
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-6">
        {filteredIngresos.length === 0 ? (
           <div className="text-center py-20 bg-white/50 dark:bg-slate-900/50 border-2 border-dashed border-brand-200 dark:border-slate-800 rounded-3xl transition-colors">
             ...
             <p className="text-brand-500 dark:text-slate-400 font-medium text-sm max-w-sm mx-auto">No se encontraron acuerdos de honorarios variables que coincidan con tu búsqueda actual.</p>
           </div>
        ) : (
                      ...
                      <div className="p-5 pb-4 border-b border-brand-50 dark:border-slate-800/50 flex justify-between items-start gap-4">
                         <div className="flex-1">
                           <span className={\`inline-block px-2 py-0.5 text-[9px] font-black uppercase tracking-wider border rounded-md whitespace-nowrap mb-2 \${getTypeColor(ingreso.tipo)}\`}>
                             {ingreso.tipo === 'Obligaciones por Acto Procesal' ? 'Acto Procesal' : 'Audiencia'}
                           </span>
                      ...
                      <div className="p-5 flex-1 flex flex-col gap-4">
                         <div className="flex flex-wrap items-end justify-between gap-y-2">
                            <div>
                               ...
                               {ingreso.montoGlobalObligacion > 0 && (
                                 <div className="text-[10px] font-bold text-orange-600 dark:text-orange-400 mt-1">Estimado: {formatCurrency(ingreso.montoGlobalObligacion)}</div>
                               )}
                            </div>
...
console.log("Successfully replaced variables table with grid!");
